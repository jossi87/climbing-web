    ...
    (files: (File & { path: string })[]) => {
      ...
      reader.onload = (e) => {
        ...
        if (coords.length >= 2 && parking) {
          ...
          const distanceFromParkingToFirstPoint = calculateDistanceBetweenCoordinates(
            parking.latitude ?? 0,
            parking.longitude ?? 0,
            coords[0].latitude ?? 0,
            coords[0].longitude ?? 0,
          );
          const distanceFromParkingToLastPoint = calculateDistanceBetweenCoordinates(
            parking.latitude ?? 0,
            parking.longitude ?? 0,
            coords[coords.length - 1].latitude ?? 0,
            coords[coords.length - 1].longitude ?? 0,
          );
          if (distanceFromParkingToLastPoint < distanceFromParkingToFirstPoint) {
            coords.reverse();
          }
        }
        onChange(coords);
    ...
    panes.push({
      ...
      render: () => (
        <div {...getRootProps()} style={{ textAlign: 'center' }}>
          <Message>
            <Message.Content>
              Drag-and-drop a <code>.gpx</code>/<code>.tcx</code> file to create the approach path.
              These can be obtained from GPS watches or from{' '}
              <a
