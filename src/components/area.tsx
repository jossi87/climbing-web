...
const Area = () => {
  ...
  if (markers.length>0 || outlines.length>0 || (data.lat && data.lat>0)) {
    ...
    panes.push({
      ...
      render: () => <Tab.Pane><Leaflet height={height} markers={markers} outlines={outlines} polylines={polylines} legends={null} defaultCenter={defaultCenter} defaultZoom={defaultZoom} history={history} onClick={null} onlyMap={false} showPhotoNotMap={false} /></Tab.Pane>
    });
  }
