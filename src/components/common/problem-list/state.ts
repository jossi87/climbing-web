...
const SORTS: Record<State["order"], (a: Row, b: Row) => number> = {
  ...
  ascents: (a, b) =>
    b.numTicks - a.numTicks || a.name.localeCompare(b.name, getLocales()),
  ...
  number: (a, b) =>
    (a?.nr ?? 0) - (b?.nr ?? 0) || a.name.localeCompare(b.name, getLocales()),
...
export const useProblemListState = ({
  ...
  const [storedOnlyFa, setStoredOnlyFa] = useSessionStorage(
    `problemList/${key}/onlyFa`,
    false,
  );
  const [storedGroupBy, setStoredGroupBy] = useSessionStorage(
    `problemList/${key}/groupBy`,
    "none",
  );
  ...
  const [
    { gradeLow, gradeHigh, hideTicked, onlyFa, order, groupBy, types },
    dispatch,
    ...
    onlyFa: storedOnlyFa,
    types: rows.reduce(
      (acc, { subType }) => ({ ...acc, [subType]: false }),
      {},
    ),
  });

  useEffect(
    () => setStoredHideTicked(hideTicked),
    [hideTicked, setStoredHideTicked],
  );
  ...
  const [filtered, uniqueAreas, uniqueRocks, uniqueSectors, uniqueTypes] =
    useMemo(() => {
