...
const ProblemSection = ({ sections : initSections, grades, onSectionsUpdated }) => {
  ...
  function onNumberOfSectionsChange(e, { value }) {
    ...
    if (num > 1) {
      ...
      while (num > newSections.length) {
        newSections.push({id: newSections.length*-1, nr: newSections.length+1, grade: 'n/a', description: null});
      }
      while (num < newSections.length) {
        newSections.pop();
      }
  ...
  return (
    <Segment>
      ...
      {sections && sections.length > 1 && sections.map((s, i) => (
        <Form.Group widths='equal' key={i} inline>
          <Form.Field>
            <Input size="mini" icon="hashtag" iconPosition="left" fluid placeholder='Number' value={s.nr} onChange={(e, { value }) => {
              s.nr = parseInt(value);
          ...
          <Form.Field>
            <Dropdown size="mini" icon="dropdown" fluid selection value={s.grade} onChange={(e, { value }) => {
              s.grade = value;
              setSections([...sections]);
              onSectionsUpdated(sections);
            }} options={grades.map((g, i) => ({key: i, value: g.grade, text: g.grade}))} />
          </Form.Field>
          <Form.Field>
            <Input size="mini" icon="info" iconPosition="left" fluid placeholder='Description' value={s.description? s.description : ""} onChange={(e, { value }) => {
              s.description = value;
              setSections([...sections]);
              onSectionsUpdated(sections);
            }} />
