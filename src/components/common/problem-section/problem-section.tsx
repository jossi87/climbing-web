...
const ProblemSection = ({ sections: initSections, onSectionsUpdated }: Props) => {
  ...
  const onNumberOfSectionsChange = useCallback(
    (e: ChangeEvent<HTMLSelectElement>) => {
      ...
      if (
        !confirm(`Are you sure you want to change number of pitches from ${currentLen} to ${num}?`)
      ) {
        return;
      }
      ...
      {sections && sections.length > 1 && (
        <div className='space-y-3'>
          {sections.map((s, index) => (
            <div
              key={index}
              className='grid grid-cols-1 sm:grid-cols-12 gap-3 items-start pb-3 border-b border-surface-border/50 last:border-0 last:pb-0'
            >
