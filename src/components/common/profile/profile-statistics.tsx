...
import { Icon, List, Label, Divider, Button, Tab, TabPane, Message } from 'semantic-ui-react';
import {
  numberWithCommas,
  downloadUsersTicks,
  useAccessToken,
    ...
    <List.Header>
      ...
      {tick.subType ? (
        <Label
          ...
          detail={(tick.numPitches ?? 0) > 1 ? tick.numPitches + ' pitches' : null}
        />
      ) : null}{' '}
      {tick.comment && (
        <small style={{ color: 'gray' }}>
...
const ProfileStatistics = ({
  ...
  const regions = Array.from(
    new Set(data.ticks?.map((t: components['schemas']['ProfileStatisticsTick']) => t.regionName)),
  ).sort();
