...
const SvgEdit = () => {
  ...
  const [readOnlySvgs, setReadOnlySvgs] = useState();
  const [activePoint, setActivePoint] = useState();
  const [draggedPoint, setDraggedPoint] = useState();
  const [draggedCubic, setDraggedCubic] = useState();
  ...
  return (
    <Container onMouseUp={cancelDragging} onMouseLeave={cancelDragging}>
      ...
      <svg viewBox={"0 0 " + w + " " + h} onClick={handleOnClick} onMouseMove={handleMouseMove} width="100%" height="100%">
        <image ref={imageRef} xlinkHref={getImageUrl(mediaId)} width="100%" height="100%"/>
