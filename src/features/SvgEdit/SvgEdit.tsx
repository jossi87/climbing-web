...
export const SvgEdit = ({
  ...
  // We use a non-passive touchstart listener to call preventDefault() when touching a point,
  // which stops the browser from starting scroll/pan on that specific touch.
  useEffect(() => {
  ...
  // reliably stops the browser from starting its scroll gesture.
  useEffect(() => {
    const svg = svgRef.current;
    if (!svg) return;

    const onTouchStart = (e: TouchEvent) => {
      // Walk up from the touch target to find data-point-index
      let el = e.target as Element | null;
      while (el && el !== svg) {
        if (el.getAttribute('data-point-index') !== null) {
          e.preventDefault();
          return;
        }
        el = el.parentElement;
