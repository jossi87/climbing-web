...
export function isPathVisible(path: string, mediaRegion: MediaRegion | null): boolean {
  if (!mediaRegion) {
    return true;
  }
...
export function scalePath(path: string, mediaRegion: MediaRegion | null): string {
  if (!mediaRegion) {
    return path;
  }
  const pathLst = path.replace('  ', ' ').trim().split(' ');
