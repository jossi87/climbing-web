...
export const SearchInput = ({ isPending, className, onClear, ...props }: SearchInputProps) => {
  ...
  return (
    <div className='group relative w-full rounded-lg text-slate-400 transition-colors group-focus-within:text-slate-200 focus-within:ring-1 focus-within:ring-slate-500/40'>
      <div className='pointer-events-none absolute inset-y-0 left-0 z-10 flex items-center pl-3 text-slate-500 transition-colors group-focus-within:text-slate-300'>
      ...
      {showClear ? (
        <button
          ...
          className='hover:bg-surface-raised-hover absolute top-1/2 right-1 z-10 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-md text-slate-500 transition-colors hover:text-slate-200 focus-visible:ring-2 focus-visible:ring-slate-400/50 focus-visible:outline-none'
