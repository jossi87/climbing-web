      ...
      {accordionRows.map((d, i) => {
        ...
        return (
          <div key={d.label}>
            ...
            >
              <span className='text-[13px] font-medium tracking-tight text-slate-400 md:text-[14px]'>{d.label}</span>
              <ChevronDown
                size={14}
                strokeWidth={2}
                className={cn(
            ...
            >
              <div className='px-2.5 pb-2.5 pl-3 sm:px-3 sm:pb-3'>
                {hasData ? d.content : <i className='text-[13px] text-slate-400 md:text-[14px]'>No data</i>}
              </div>
            </div>
          </div>
        );
