...
const MediaModal = ({
  ...
  /**
   * Warm the on-demand high-res variants while the modal is open, so the zoom viewer
   * (and the adjacent carousel items) are already generated by the time the user asks
   * for them. Deferred so the warm-up never competes with the currently-visible image.
   */
  useEffect(() => {
    const entries: { id: number; stamp: number; width: number }[] = [];
    const seen = new Set<number>();
    const consider = (item?: components['schemas']['Media'] | null) => {
      if (!item) return;
      const id = mediaIdentityId(item.identity);
      if (id <= 0 || seen.has(id)) return;
      seen.add(id);
      entries.push({
        id,
        stamp: mediaIdentityVersionStamp(item.identity),
        width: Math.max(Number(item.width ?? 0) || 0, 2560),
      });
    };
    consider(m);
    const currentIndex = orderableMedia.findIndex((x) => mediaIdentityId(x.identity) === mediaIdentityId(m.identity));
    if (currentIndex >= 0) {
      consider(orderableMedia[(currentIndex - 1 + orderableMedia.length) % orderableMedia.length]);
      consider(orderableMedia[(currentIndex + 1) % orderableMedia.length]);
    }
    const timer = window.setTimeout(() => {
      for (const e of entries) {
        const img = new Image();
        img.src = getMediaFileUrl(e.id, e.stamp, false, { targetWidth: e.width });
      }
    }, 800);
